var AIRUpdater = function () {
	...
	var applicationName = "Twutulater.air";
	...
	*/
	var latestVersionCheckUrl = "http://twitulater.com/versionData.xml";
	...
	*/	
	var updaterUrl = "http://twitulater.com/downloader.php?file=repo/Twitulater_";
	var stream = null;
	var updateFile = null;
	...
	var getApplicationVersion = function () {
		...
		applicationVersion = xmlObject.getElementsByTagName('version')[0].firstChild.nodeValue;
	};
			...
			if (XMLHttp.readyState === 4) {
				...
				*/	
				if (typeof latestVersionNode === "object" && latestVersionNode.firstChild) {
					latestVersion = latestVersionNode.firstChild.nodeValue;
					compareVersions();
				}
			}
		};
	...
	var compareVersions = function () {
		if (/*applicationVersion > 0 && latestVersion > 0 && */latestVersion > applicationVersion) {
			...
			*/
			...
			if (confirm(Language.def("updater_warning")+"\n"+Language.def("updater_features")+"\n"+releaseNotesText+"\n"+Language.def("updater_question"))==true) {
